...
data_dirs = {'train': '/home/ubuntu/projects/deepsolar/dataset/HR/train',
             'val': '/home/ubuntu/projects/deepsolar/dataset/HR/val'}
...
ckpt_save_dir = 'checkpoint/deepsolar_HR_5'

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
trainable_params = ['fc']     # layers or modules set to be trainable. "None" if training all layers
model_name = 'deepsolar'  # the prefix of the filename for saving model/checkpoint
...
if __name__ == '__main__':
    # data
    image_datasets = {x: datasets.ImageFolder(data_dirs[x], data_transforms[x]) for x in ['train', 'val']}
    ...
    if old_ckpt_path:
        ...
        if old_ckpt_path[-4:] == '.tar':  # it is a checkpoint dictionary rather than just model parameters
            ...
            # del checkpoint['AuxLogits.fc.weight']  # delete the unused parameters
            # del checkpoint['AuxLogits.fc.bias']  # delete the unused parameters
            # del checkpoint['fc.weight']  # delete the unused parameters
            # del checkpoint['fc.bias']  # delete the unused parameters
