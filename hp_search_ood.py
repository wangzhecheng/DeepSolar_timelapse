...
ckpt_save_dir = 'checkpoint/ood/ood_all_synPartial_LR_012_HR_12_resnet50'
...
def metrics(stats):
    ...
    recall = (stats['00'] + 0.00001) * 1.0 / (stats['00'] + stats['01'] + stats['02'] + 0.00001)
    precision = (stats['00'] + 0.00001) * 1.0 / (stats['00'] + stats['10'] + stats['20'] + 0.00001)

    spec = (stats['11'] + 0.00001) * 1.0 / (stats['11'] + stats['12'] + 0.00001)
    sens = (stats['22'] + 0.00001) * 1.0 / (stats['22'] + stats['21'] + 0.00001)

    m = 4.0 * spec * sens * precision * recall / (precision * recall * spec + precision * recall * sens +
                                                     precision * spec * sens + recall * spec * sens + 1e-7)
    return m, recall, precision, sens, spec
    ...
    for epoch in range(current_epoch, current_epoch + num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for ll in range(nclasses):
                for pp in range(nclasses):
                    stats[str(ll) + str(pp)] = 0
            ...
            for inputs, labels in tqdm(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        ...
                        for ll in range(nclasses):
                            for pp in range(nclasses):
                                stats[str(ll)+str(pp)] += torch.sum((preds == pp) * (labels == ll)).cpu().item()
            ...
            if phase == 'val':
                ...
                epoch_max_metrics, recall, precision, sens, spec = metrics(stats)

                if verbose:
                    print('{} Loss: {:.4f} Metrics: {:.4f} Threshold: {:.4f} Recall: {:.4f} Precision: {:.4f} Sensitivity: {:.4f} Specificity: {:.4f}'.format(phase, epoch_loss,
                                                                                     epoch_max_metrics, epoch_best_threshold, recall, precision, sens, spec))
...
if __name__ == '__main__':
    ...
    results_dict = {x: {y: {z: {} for z in weight_decay_list} for y in lr_decay_epochs_list} for x in ib1_list}
    ...
    for ib1 in ib1_list:
        for lr_decay_epochs in lr_decay_epochs_list:
            for weight_decay in weight_decay_list:
                ...
                class_weight = torch.tensor([ib1, 1, 1], dtype=torch.float).cuda()
                ...
                best_model, _, best_metric_value, best_threshold = train_model(model, model_name=model_name + '_ib1_' + str(
                    ib1) + '_decay_' + str(lr_decay_epochs) + '_wd_' + str(weight_decay),
                ...
                print('Begin test ...')
                test_stats, test_metric_value = test_model(best_model, dataloaders_dict['test'], metrics, threshold_list=threshold_list)

                results_dict[ib1][lr_decay_epochs][weight_decay] = {'best_metrics_val': best_metric_value,
                                                                              'metric_test': test_metric_value,
